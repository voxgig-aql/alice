...
from AQL itself. The `aless` viewer compensates by exporting its update
fold (`Aless.feed state ev → state'`) and driving it from a plain test
...
an event and read the screen/frame — so the `Aless.feed` pattern can
