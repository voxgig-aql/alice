...
alice.aql + viewer/        alice — a jless-style TUI file viewer written
                           in AQL (tabs, watch-reload); see viewer/README.md.
                           Tracks aql MAIN, unlike the pinned library.
test/alice_*.aql           the viewer's suites (+ test/fixtures/)
```
...
A separate `viewer` job builds the latest aql *main* and runs the `alice`
...
```bash
aql alice.aql                  # the viewer itself (any terminal)
aql test/alice_app_unit_test.aql  # …and its headless integration suite
```
